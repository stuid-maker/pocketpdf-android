...
class SummarizeDocumentUseCaseCacheTest {
    ...
    private val useCase = SummarizeDocumentUseCase(
        retrieveChunks, documentRepository, llmRepository, summaryCacheStore,
    )

    @Test
    fun `returns cached value when cache hits for Full scope`() = runTest {
        every { summaryCacheStore.get(1L, SummaryScope.Full) } returns flowOf("缓存的全文总结")
    ...
    fun `returns cached value when cache hits for Page scope`() = runTest {
        every { summaryCacheStore.get(1L, SummaryScope.Page(2)) } returns flowOf("缓存的第3页总结")
    ...
    fun `cache hit does not call LLM`() = runTest {
        every { summaryCacheStore.get(1L, SummaryScope.Full) } returns flowOf("cached")
