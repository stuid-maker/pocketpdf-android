...
> 第 2 周：本地 RAG 数据管线（文本切块与本地向量化）。**当前进度：Day 1 计划中**。

## 1. 本周目标（来自 ROADMAP）

- [ ] 分支与环境收尾：合并 W1，打 `v0.1.0-pdf-reader` 标签，开启 `feat/local-rag-indexing` 功能分支（**Day 1 目标**）
- [ ] Room 表扩展：`ChunkEntity` + 向量数据类型转换（TypeConverter），Schema 升至 v2（**Day 1 目标**）
- [ ] 领域模型：`DocumentChunk` 实体与 Repository 接口扩展（**Day 1 目标**）
