...
>
> **Day 0-3 实际**（2026-05-20）：D0-D3 在同一天全部完成。D0 前置检查通过（模型 6.1MB、LM Studio 在线、42 测试基线绿），D1-D3 代码实现 + 25 个新增测试全绿 + 端到端 LLM 流式验证通过。W3 核心 UseCase 全部就绪，待 D4 接 UI。
...
- [x] `SummarizeDocumentUseCase`（MapReduce：每 chunk 出小结 → 合并）
- [ ] 阅读器顶部按钮："总结本页" / "总结全文"
- [ ] 总结结果浮层 + 流式打字效果 + 复制按钮
- [ ] 设置页：LLM Base URL、模型名、可选 API Key
- [x] 单元测试：检索排序、MapReduce 合并
