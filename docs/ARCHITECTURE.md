...
> 本文随项目演进同步更新。W5 完整版，包含所有子系统的 Mermaid 图表。

最近更新：Week 5 · 2026-06-01
...
| 网络 | Retrofit 2.11 + OkHttp 4.12 | SSE 流式走原生 OkHttp |
| PDF 渲染 | Android PdfRenderer + 自定义 PdfPageView | Bitmap + Matrix 缩放 |
| PDF 文本 | PdfBox-Android 2.0.27 | 按页提取文本 |
...
graph TD
    ...
    DI -.provides.-> DATA
    DATA[data<br/>Room/Retrofit/OkHttp/PdfBox<br/>MediaPipe/WorkManager] --> DOMAIN
...
graph LR
    ...
    Copy -->|File| Parse[PdfBoxTextExtractor<br/>extractPagesText]
    Parse -->|List<String> 按页| Room[(Room DB<br/>DocumentEntity)]
    Copy -->|内部存储 File| Render[ReaderActivity]
    
    Render -->|ParcelFileDescriptor| PdfRenderer[Android PdfRenderer]
    PdfRenderer -->|Bitmap| PdfPageView[PdfPageView<br/>自定义 View]
...
#### PdfBoxTextExtractor 流程

```
file → PDDocument.load() → PDFTextStripper(逐页 startPage=endPage) → List<String>
```

- 使用 `PDFTextStripper` 按页提取（非全文 strip 后拆分——对扫描件/多列布局有更好支持）
- 全部 `withContext(dispatchers.io)`：PDDocument.load + getText 都是阻塞操作
- try/finally 保证 `document.close()`（避免 fd 泄漏）
...
graph TD
    ...
    subgraph 索引编排
        ...
        IW -->|Step 2| ExtractText[PdfBox 逐页提取]
...
- **理由**：
  ...
  4. **Reader 已有完整实现**：PdfPageView 自定义 View、PdfRenderer 等与 XML View system 深度绑定，迁移到 Compose 需要考虑 Canvas 互操作，新功能增量大于迁移收益
...
graph TD
    ...
    subgraph "Data Layer"
        ...
        NW[OkHttp + Retrofit<br/>LlmApi/SseStreamParser]
