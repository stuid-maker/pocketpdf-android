...
| UI | Jetpack Compose + Material 3 | Library、Reader、Chat、Settings、Diagnostics |
| PDF Canvas | 自定义 `PdfPageView` + `AndroidView` | 缩放、平移、翻页、搜索与标注绘制 |
| DI | Hilt 2.52 | @HiltAndroidApp + @HiltViewModel |
| 数据库 | Room 2.6.1 | Schema v6，含文档、chunk、聊天与标注 |
| 网络 | OkHttp 4.12（原生，无 Retrofit） | SSE 流式、模型列表、连接测试统一走 OkHttp |
| PDF 阅读/搜索/文本 | PdfiumAndroid 1.0.30 + 自定义 PdfPageView | 同一引擎负责渲染、文本和搜索坐标；因 Kotlin 2.0 ABI 固定版本 |
| PDF 兼容路径 | PdfBox-Android 2.0.27 | 仅保留旧的长按标注字符位置能力 |
| 向量化 | MediaPipe TextEmbedder (on-device) | Universal Sentence Encoder `.tflite` |
| LLM 后端 | LM Studio (Gemma 3 4B-IT) | OpenAI 兼容协议，adb reverse 桥接 |
| 后台任务 | WorkManager | 文档索引用 OneTimeWorkRequest |
| 测试 | JUnit4 + MockK + Turbine + Robolectric + Compose/Espresso | 402 JVM + 31 Android |
...
```mermaid
graph TD
    UI[ui<br/>Activity/ViewModel<br/>Compose Screen/PdfPageView] --> DOMAIN
...
graph TD
    ...
    subgraph LLM 桥接
        ...
        Parser -->|callbackFlow worker| Flow[Flow~String~]
    end
...
- 模型列表、连接测试与流式生成全部走原生 OkHttp，并动态读取设置中的 base URL。
- `chatCompletionStream` 在 `callbackFlow` 子协程中执行阻塞请求；`awaitClose` 取消 `Call`、关闭响应并取消 worker。
- 原生 OkHttp 直接读取 `ResponseBody.source()`，避免缓冲完整 SSE 响应。
...
- `SseStreamParser` 逐行解析；repository 的 `callbackFlow` worker 负责 IO、生命周期和取消
...
### ADR-001: 用 XML 而非 Compose（已被 ADR-007 取代）

- **状态**：历史决策。v1.1.0 已完成全屏 Compose 迁移，仅保留 `PdfPageView` 作为专用 Canvas View。
...
### ADR-005: 为什么 Chat 用 Compose 但 Reader 用 XML（已被 ADR-007 取代）

- **状态**：历史决策。后续 Purple Crystal 迁移证明全屏 Compose 的统一维护收益已超过迁移成本。
...
### ADR-007: 全屏 Compose + 专用 PDF Canvas View

- **背景**：v1.1.0 统一了 Library、Reader、Chat、Settings 与 Diagnostics 的视觉和状态模型。
- **决策**：屏幕与导航 chrome 使用 Compose；PDF 位图、矩阵缩放、搜索高亮与标注绘制继续由 `PdfPageView` 负责，并通过 `AndroidView` 承载。
- **理由**：
  1. Compose 统一页面布局、主题、状态和测试。
  2. Canvas/Matrix 手势代码已稳定，封装为专用 View 比重写更低风险。
  3. 标准 `performClick()`、生命周期回收和回调边界让互操作保持可访问、可测试。
- **日期**：2026-06-08；2026-06-13 补充发布验收。
...
- **代价**：
  ...
  - 需要打包 `.tflite` 模型文件到 APK（6,120,274 bytes）
